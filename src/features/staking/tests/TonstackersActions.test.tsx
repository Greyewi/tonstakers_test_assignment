...
describe('TonstackersManageUI', () => {
  ...
  it('renders StakeForm and UnstakeForm correctly', () => {
    ...
    expect(
      screen.getByPlaceholderText('Enter amount to stake')
    ).toBeInTheDocument()
    expect(
      screen.getByPlaceholderText('Enter amount to unstake')
    ).toBeInTheDocument()
  })
  ...
  it('displays insufficient staked balance message for unstaking when unstake amount exceeds tsTonStacked', () => {
    ...
    const unstakeButton = screen.getByRole('button', {
      name: /unstake 60 tsTONs/i
    })
