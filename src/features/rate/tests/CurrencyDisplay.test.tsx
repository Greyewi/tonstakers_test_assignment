...
describe('CurrencyRate', () => {
  ...
  it('displays currency data correctly when loaded', () => {
    ...
    expect(screen.getByAltText('TON logo')).toHaveAttribute(
      'src',
      mockTonData.image_url
    )
    expect(
      screen.getByText(
        `Price: $${Number(mockTonData.dex_usd_price).toFixed(Number(import.meta.env.VITE_FIXED_DECIMALS))}`
      )
    ).toBeInTheDocument()

    expect(screen.getByText('tsTON')).toBeInTheDocument()
    expect(screen.getByAltText('tsTON logo')).toHaveAttribute(
      'src',
      mockTsTonData.image_url
    )
    expect(
      screen.getByText(
        `Price: $${Number(mockTsTonData.dex_usd_price).toFixed(Number(import.meta.env.VITE_FIXED_DECIMALS))}`
      )
    ).toBeInTheDocument()
  })
